...
PROJECT=<id> ZONE=us-central1-a ./setup-gcp.sh   # idempotent VM create (see below for what is actually running)
...
* **Search is OFF; the corpus is not.** `AO_SEARCH_DISABLED=1` (set in `deploy/docker-compose.yml`, read by `api/app/config.py`) makes `/v1/search`, `/public/search` and `/internal/search` answer 503 with a clear reason, and the **Qdrant container has been removed from the compose file**. The reasoning is worth keeping: search was the only thing on the box needing the vector index, and the index was the only thing needing a big machine — 2.45M x 384-d vectors plus the HNSW graph, capped at 5g against 2g each for Postgres and the API. Nothing was calling `/v1/search`. The `/q` pages that carry all ~200k monthly impressions are a plain Postgres row lookup (`run_get_doc`) and never touched Qdrant, so the corpus stays fully served on a far smaller instance. `/v1/health` reports `ok` from Postgres alone plus `"search": "disabled"` — folding a missing Qdrant into `ok` would report the whole deployment down while every page served fine. The `qdrant_data` volume is still declared on purpose so an existing VM's index is not destroyed; restoring the service block from git history and clearing the flag brings search back with no code change. Thalamus-side, `backendDownResult` maps this 503 to `search_disabled` rather than `backend_unavailable`, so it never reads as an outage.
* **What is actually deployed (checked against the Compute API, project `iamnitishgoel`, not inferred from this file):** instance `agentoverflow` in `us-central1-a`, **`e2-standard-2`**, provisioning model **`STANDARD` — not Spot**. Disks: 30 GB boot (pd-balanced), `ao-data` 150 GB (pd-balanced), `ao-scratch` 300 GB (pd-standard). A `ao-daily-snap` policy snapshots `ao-data` daily with 7-day retention. Earlier revisions of this file said "e2-standard-4 SPOT"; that described the ingestion era and was stale on both counts.
* **Standard, not Spot, is the right call now — do not "fix" it back.** Spot was correct while this box only ran ingestion, where every stage is resumable and a preemption costs nothing. It serves 2.45M live `/q` pages now, and a preemption means 503s on all of them; `functions/q/[docId].js` fails closed by design, so sustained 503s are the deindexation trigger for the whole corpus. The ~$34/mo premium buys exactly that.
* **The bill is disk, not compute.** The daily snapshots report `storageBytes` of 9.3 GB for the first and **0.0 GB for every delta since** — the corpus is static and nothing writes to it. So ~480 GB of provisioned disk is holding under 10 GB of real data. `ao-scratch` (300 GB) exists solely for the ~100 GB `Posts.xml` stream and the ingest is complete; `ao-data` is 150 GB provisioned for ~9 GB used. Neither shrinks on its own — GCP cannot shrink a disk in place, so reclaiming `ao-data` means creating a smaller disk from a snapshot and swapping it in.
* **Spot VM (historic, applies only if ever moved back):** preemption stops (not deletes) the instance and every stage resumes — but stop/start rotates the ephemeral IP. The static-IP promotion in RUNBOOK step 8 is what keeps `AO_VM_URL` from silently breaking.
